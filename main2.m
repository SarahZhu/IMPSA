...
for sub_len = 4:step:ts_len %sl_temp:1:sl_temp       %sub_len = 4:step:ts_len
    ...
    for i = 1:length(shapelet_idxs)
        ...
        if abs(bsf_accuracy-1.0)< tol % Early abandonto save time, added by Sarah 
           break
        end
    end
    % update global best shapelet info
    if bsf_glb_accuracy < bsf_accuracy 
    ...
    if abs(bsf_glb_accuracy-1.0)< tol % Early abandonto save time, added by Sarah 
       break
    end
end
